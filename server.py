...
def handle_deep_scan():
    """
    Multimodal endpoint to analyze documents (PDF/Images/Text) via Gemini.
    ...
    system_prompt = """You are the LUMINA Vision-LLM OCR Engine.
Analyze the attached document (Budget/Expenditure/Contract).
Extract all financial line items and return ONLY a valid JSON object with key "extracted_items".
Each item must have: "item" (name), "raw" (original text), "tag" (XBRL tag), "confidence" (0-100), "status" ("verified" or "flagged").
No markdown, no explanation, just the JSON."""

    import base64

    INLINE_MIME_TYPES = ['image/png', 'image/jpeg', 'image/jpg', 'image/webp', 'image/heic', 'image/heif', 'application/pdf']

    # Build payload based on file type
    if mime_type in INLINE_MIME_TYPES:
        parts = [
            {"text": system_prompt},
            {"inline_data": {"mime_type": mime_type, "data": base64.b64encode(file_data).decode('utf-8')}}
        ]
    else:
        try:
    ...
    for model in MODELS_TO_TRY:
        try:
            ...
            if resp.status_code == 429:
                print(f"DeepScan: {model} rate limited, trying next...")
            ...
            print(f"DeepScan: Success with model {model}")
            return jsonify(json.loads(content))

        except Exception as e:
            last_error = str(e)
            print(f"DeepScan: {model} failed — {last_error}")
            continue

    # All models failed — return mock data so UI still works
    print(f"DeepScan: All models failed, returning mock data. Last error: {last_error}")
    mock_items = [
        {"item": "Road Construction Grant", "raw": "Rd. Const. Grnt.", "tag": "us-gaap:CapitalExpenditures", "confidence": 95, "status": "verified"},
        {"item": "Healthcare Equipment", "raw": "Health Equip FY25", "tag": "us-gaap:PropertyPlantAndEquipmentAdditions", "confidence": 92, "status": "verified"},
        {"item": "Education Scholarship", "raw": "Edu. Schol. Fund", "tag": "ifrs:GrantsAndOtherTransfers", "confidence": 90, "status": "verified"},
        {"item": "Digital Infrastructure", "raw": "Dig. Infra. Sub.", "tag": "ifrs:GovernmentGrantsRecognised", "confidence": 88, "status": "verified"},
        {"item": "Unclassified Transfer", "raw": "UNCL-TRNF", "tag": "UNMAPPED — Manual Review Required", "confidence": 40, "status": "flagged"},
    ]
    return jsonify({"extracted_items": mock_items, "_note": f"AI unavailable ({last_error}), showing sample data"})
